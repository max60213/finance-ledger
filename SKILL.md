...
10. Treat `Subscription`, `Investment`, and `Savings` as preset monthly items that start as already spent/invested in monthly summaries. Translate those labels to match the user's language or the vault's existing language when writing notes.
11. Keep category names stable and prefer the vault's existing categories by default: `Food`, `Transport`, `Other`, plus `Subscription`, `Investment`, and `Savings` when summarizing a month. Translate labels to match the user's language or the vault's existing language when writing notes.
